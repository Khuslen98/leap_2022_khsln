...
objLoader.load("Astronaut.obj", function (object) {
  object.traverse(function (child) {
    if (child instanceof THREE.Mesh) {
      ...
      createVerticles(spacemanParticles, spacemanPoints, yOffset, 3);
    }
  });
});
...
for (var p = 0; p < particleCount; p++) {
  ...
  vertex.y = 0;
  vertex.z = 0;
  particles.verticles.push(vertex);
}
createVerticles(sphereParticles.spherePoints, null, null);
createVerticles(cubeParticles, cubePoints, null, 1);

function createVerticles(emptyArray, points, yOffset = 0, trigger = null) {
  for (var p = 0; p < particleCount; p++) {
    var vertex = new THREE.Vector3();
    vertex.x = points[p]["x"];
    ...
    emptyArray.verticles.push(vertex);
  }
  if (trigger !== null) {
...
const normalSpeed = defaultAnimatioSpeed / 100,
...
function morphTo(newParticles, color = "0xffffff") {
  ...
  for (var i = 0; i < particles.verticles.length; i++) {
    TweenMax.to(particles.verticles[i], 4, {
      ease: Elastic.easeOut.config(1, 0.75),
      x: newParticles.verticles[i].x,
      y: newParticles.verticles[i].y,
      z: newParticles.verticles[i].z,
    });
  }
}
function slowDown() {
